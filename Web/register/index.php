...
else if($credentials['facebookId'] != NULL && $credentials['password'] != NULL)
{
	$db = new DbConnect();
	$query = "Insert into users(facebookId, password, username) values(:facebookId, :password, :username)";
	$credentials['password'] = password_hash($credentials['password'], PASSWORD_BCRYPT);
	try
	{
		if($db->execute_query($query, $credentials))
		{
			http_response_code(200);
			echo "User ".$credentials['facebookId']." created";
		}
		else
		{
			http_response_code(400);
